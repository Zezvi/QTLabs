...
Window {
    ...
    title: qsTr("Редактирование информации о марке")
    minimumWidth: 400
    maximumWidth: 400
    minimumHeight: 200
    maximumHeight: 200
    ...
    Button {
        visible: {textName.length>0 && textCountry.length>0 && root.currentIndex <0}
    ...
    Button {
        ...
        onClicked: {
            root.hide()
            edit(textName.text, textCountry.text, textYear.text, textCopies.text, root.currentIndex)
        }
    }
    Button {
        id: buttonCancel
    ...
    function execute(name, country, year, copies, index){
        textName.text = name
        textCountry.text = country
        textYear.text = year
        textCopies.text = copies
